...
function checaTipoPromocao($promo, $tipo){

	if($promo == "personalizado"){
		$temp = "personalizado";
	}elseif($promo == "brinde"){
		$temp = "cupom-brinde.png";
	}else{
		$temp = null;
	}
	...
	if($temp == null){
				return '<div class="novobotao'.$hifen.'aberto">'.$promo.'</div>';
			}else{
				return '<div class="novobotao'.$hifen.$promo.'"></div>';
			}
...
function checaItem($linha){

	if( $GLOBALS['total_utilizacoes'] >= 1){ echo'<td align="center"><p class="utilizado">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p></td>'; }
					elseif($linha['premio'] == null) { }
					elseif( $linha['premio'] == false){
					echo'<td align="center"><a class="numero-tabela" href="codigo.php?cartao_id='.$GLOBALS['cartao_id'].'">0'.$item.'</a></td>'; }
					elseif($linha['premio'] != null){ echo'<td align="center"><a class="numero-tabela" href="codigo.php?cartao_id='.$GLOBALS['cartao_id'].'"><p class="brinde">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p></a></td>'; }
...
/*
**
** GERA O NOVO CÓDIGO CASO O CLIENTE
** AINDA NÃO POSSUA UM E INSERE NO BANCO.
**
*/
// Script que vai gerar o código automático

function geraCodigo($tamanho = 6, $maiusculas = false, $simbolos = false, $texto = false){
...
function checaCodigoAberto($usuario, $cartao, $codigo, $tipo, $con){

	// cria a instrução SQL que vai selecionar os dados
	$query_utilizado = sprintf("SELECT cad_validado from clientes_codtemp WHERE user_id = ".$usuario." AND cartao_id = ".$cartao." AND cod_temporario='".$codigo."' AND tipo='".$tipo."' LIMIT 1");

	// executa a query
	$dados_utilizado = mysql_query($query_utilizado, $con) or die(mysql_error());
	$linha_utilizado = mysql_fetch_assoc($dados_utilizado);

	return $linha_utilizado['cad_validado'];

}


function checaItensCartao($cartao, $con, $user){

		// cria a instrução SQL que vai selecionar os dados
	$query = sprintf("SELECT `item_1`, `item_2`, `item_3`, `item_4`, `item_5`, `item_6`, `item_7`, `item_8`, `item_9`, `item_10`, `item_11`, `item_12` FROM cartoes WHERE cartao_id=".$cartao." LIMIT 1");
...
function salvaUtilizacao($usuario, $cartao, $colaborador, $item, $tipo, $con, $premio){


	// cartao_id é o cartão OU cupom e tipo mostra qual é. ($cartao e $tipo)
	// Com isso da para consultar a empresa.

	// cria a instrução SQL que vai selecionar os dados 
	if( $tipo == 'cupom' ){

		$query_tmp2 = sprintf("SELECT * FROM cupons WHERE cartao_id = ".$cartao." "); 

	}
	if( $tipo == 'cartao'){

		$query_tmp2 = sprintf("SELECT * FROM cartoes WHERE cartao_id = ".$cartao." "); 	

	}

	// executa a query 
	$dados_tmp2 = mysql_query($query_tmp2, $con) or die(mysql_error());
	 
	// transforma os dados em um array 
	$linha_tmp2 = mysql_fetch_assoc($dados_tmp2); 



	
	$sql = "INSERT INTO `utilizacoes` (`user_id`, `cartao_id`, `colaborador_id`, `item_nmr`, `atual`, `tipo`, `empresa_id`, `premio`) 
							VALUES ( '".$usuario."' , '".$cartao."' , '".$colaborador."' , '".$item."' , '1' , '".$tipo."' , '".$linha_tmp2['empresa_id']."' , '".$premio."')";
	...
	if($salvando == false){
		...
		{
			mysql_query("UPDATE `clientes_codtemp` SET `cod_temporario` = null, `cad_validado` = 1 WHERE `user_id`=".$usuario." AND cartao_id = ".$cartao."");
		}
		else
		{
			$salvando = false;
...
function checaItemAtual($usuario, $cartao, $con){

	//Traz todas as utiluzacoes do usuario e do cartão
	$query_utilizacoes = sprintf("SELECT * FROM utilizacoes WHERE user_id=".$usuario." AND cartao_id=".$cartao." AND atual=1");

	// executa a query
	$dados_utilizacoes = mysql_query($query_utilizacoes, $con) or die(mysql_error());

	// transforma os dados em um array
	$linha_utilizacoes = mysql_fetch_assoc($dados_utilizacoes);

	// calcula quantos dados retornaram
	$total_utilizacoes = mysql_num_rows($dados_utilizacoes);

	$utilizacoes = $total_utilizacoes + 1;
...
############ FUNÇÃO PARA CRIAR CONTA COM O FACEBOOK. ###################

function criaConta($nome, $usuario, $email, $telefone = NULL, $celular = '(00)00000-0000', $facebook_id = NULL){


			// Monta uma consulta SQL (query) para procurar um usuário
			$sql = "SELECT `nome`, `usuario`, `senha`, `email` FROM `clientes` WHERE `email`='".$email."'";
			...
			if(!empty($resultado)){
				...
				?><script type='text/javascript'>alert('Ops...\nEste email já existe em nossa base de dados, tente com um novo email!\nCaso já possua conta e queira atualizá-la com seu Facebook, faça o login e atualize seus dados no menu inicial.');</script><?php

			}
			else{ //O candidato esta válido à se cadastrar, com dados e senha válida.
			...
			//Criação das variaveis necessárias para mandar o e-mail.
			$headers = "MIME-Version: 1.0\r\n";
			$headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
			$headers .= "From: nao-responda@dindinvivo.com.br";

			$mensagem = "Olá, seja bem vindo(a) e obrigado por se cadastrar.
						Aqui está a confirmação dos seus dados, não se esqueça de alterar sua senha no seu primeiro acesso!
						...
						Obrigado.";
			$assunto = "Aqui está sua senha ".$nome."!";




			//Insere dados na table
			$sql = "INSERT INTO `clientes`(`nome`, `usuario`, `senha`, `email`, `telefone`, `celular`, `facebook_id`) VALUES ('".$nome."', '".$email."', '".$senha."', '".$email."', '".$telefone."', '".$celular."', '".$facebook_id."')";
			...
			if(mysql_query($sql)){
				...
				$mailer->Username = 'nao-responda@dindinvivo.com.br'; //Informe o e-mai o completo
				$mailer->Password = '123mudar.'; //Senha da caixa postal
				$mailer->FromName = 'DinDin Vivo - Não responda'; //Nome que será exibido para o destinatário
				$mailer->From = 'nao-responda@dindinvivo.com.br'; //Obrigatório ser a mesma caixa postal indicada em "username"
				...
				<?php
					...
					alert("Ops... Houve um erro ao gravar os dados, tente novamente mais tarde!");
				</script>
					<?php  exit; }

			} //Fim do else do email existe



}
############ FIM DA FUNÇÃO PARA CRIAR CONTA COM O FACEBOOK. ############


function checaPremios($usuario, $cartao, $conexao){
	// executa a query
	$usos = mysql_query( sprintf("SELECT * FROM `utilizacoes` WHERE `user_id`=".$usuario." AND `atual` = 1  AND cartao_id=".$cartao." and tipo='cartao' ") , $conexao) or die(mysql_error());

	$num_usos = mysql_num_rows($usos)+1;
	...
	if(!empty($linha["item_".$num_usos])){
		return $linha["item_".$num_usos];
	}else{
		return NULL;
	}
